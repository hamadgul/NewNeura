 ...
 * `/work/`.
 *
 * Behaviour, reproduced from the layout this is adapted from:
 *
 *  - Click-driven filtering with pagination, not infinite scroll.
 *  - The filter pills are `<button>`s, not links. Clicking one pushes
 *    `?service=<slug>` onto history without a navigation.
 *  - The cards of a page are split across three `collectionProjects__target`
 ...
 *      → pagination
 *
 * There is no backend: filtering and pagination run client-side over the
 * bundled dataset handed in as `projects`. Nothing is fetched at runtime; only
 * the URL behaviour is reproduced (`?service=<slug>`, pushed, and read back on
 * mount so a deep link lands on the right filter).
 *
 * Feed size note: this pages at 14 and the whole NeuraGul feed is nine
 * projects, so in practice there is exactly one page and the pagination row
 * stays collapsed. The paging is kept because it is what makes the block safe
 * to grow into.
 */
...
export interface CollectionProjectsProject {
  ...
  href: string;
  /** Every service slug on the project, Applied AI sub-services included. */
  services: string[];
 ...
 * Applied AI has them) become the second row once that service is active.
 */
...
export interface CollectionProjectsProps {
  ...
  /** Defaults to the five service lines, with their project counts. */
 ...
 * not always 0 — the arrangement picks a different element per layout:
 *   L1 [1,2,3] large=0 · L2 [4,5,6] large=2 · L3 [7,8,9] large=0
 *   L4 [10,11] large=1 · L5 [12,13,14] large=0
 * The nine-project feed fills L1 through L3 and stops there, so L4 and L5 are
 * inert today and exist for the feed to grow into.
 *
...
function ProjectCard({ project, notch = false, className, sizes }: ProjectCardProps) {
  ...
  useEffect(() => {
    // Observe the anchor, never the `.ng-image-reveal` box. That box starts
...
export function CollectionProjects({
  ...
  // Read `?service=` on mount and follow back/forward. Deliberately
  ...
  // One expression covers both tiers: a top-service slug matches `topServices`,
  // a sub-service slug matches the project's own taxonomy terms.
  //
  ...
  const filtered = useMemo(() => {
    ...
    return projects.filter(
      (project) =>
        ...
        (!service ||
          project.topServices.includes(service) ||
      ...
      */}
      ...
      >
        <div id="filterHeader" className="filterApi__filterWrapper">
          ...
          {/*
            Always 55px tall, even with no sub-services showing — the source
            keeps the empty band so the grid below never shifts when a service
