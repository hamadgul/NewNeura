...
/**
 * BlockImageSlider — the horizontal image strip on `/process/`.
 *
 * INTERACTION MODEL (decided by INTERACTIONS_PROBE.json, not assumed):
 * `sliderOverTime` samples the track four times with no interaction and
 * records `transform: "none", scrollLeft: 0` every time, and `slider`
...
export function BlockImageSlider({ images, className }: BlockImageSliderProps) {
  ...
  return (
    <div className={cn("blockImageSlider ng-grid relative my-[120px] overflow-hidden", className)}>
