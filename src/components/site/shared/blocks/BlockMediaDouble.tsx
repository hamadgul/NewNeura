 ...
 *
 * Measured geometry (grid line numbers are `.ng-grid`'s: 1 = full-start,
...
export function BlockMediaDouble({ large, small, className }: BlockMediaDoubleProps) {
  ...
  useEffect(() => {
    // Observe the SECTION, never the `.ng-image-reveal` wrapper: that wrapper
  ...
  const revealClass = cn("w-full overflow-hidden ng-image-reveal", isRevealed && "is-revealed");

  return (
    <section
      ref={sectionRef}
      className={cn("blockMediaDouble ng-grid my-[100px] xl:my-[120px]", className)}
    >
      <figure
        className={cn(
