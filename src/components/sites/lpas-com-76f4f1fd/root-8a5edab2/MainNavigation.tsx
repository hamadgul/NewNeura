...
/**
 * Fixed top navigation for lpas.com — a dark pill toggle over the hero video
 * that opens a full-screen, click-driven overlay menu.
 *
 * IMPORTANT: verified at 20 scroll positions from y=0 to y=8400, `.navigationMain`
 * never changes class, background, height or shadow on the live site — it stays
 * fixed and fully transparent at every scroll position. Do NOT add scroll
 * listeners or shrink/recolor behavior here.
 */
import Link from "next/link";
import { useCallback, useEffect, useId, useRef, useState } from "react";
import { cn } from "@/lib/utils";
import { CloseIcon, LogoIcon, MenuIcon } from "@/components/sites/lpas-com-76f4f1fd/shared/icons";
...
import type { NavGroup, OfficeContact } from "@/types/lpas";
    ...
    <div className="navigationMain fixed inset-x-0 top-0 z-[1000] flex h-[100px] w-full items-center bg-transparent">
      <div className="navigationMain__topBar relative z-10 mx-auto my-[25px] flex h-[50px] w-[calc(100%-30px)] items-center justify-between py-[10px] md:w-[calc(100%-60px)] lg:w-[calc(100%-80px)]">
        <Link
          ...
          className="navigationMain__topBarLogo flex h-[30px] w-[60px] items-center text-white"
        >
          <LogoIcon className="h-[19px] w-[60px]" />
        </Link>
      ...
      >
        <div className="mx-auto grid w-full max-w-[1440px] auto-rows-min gap-x-[40px] gap-y-[48px] px-[15px] pb-[60px] pt-[130px] md:grid-cols-2 md:px-[30px] md:pt-[150px] lg:grid-cols-[1fr_1.4fr_1fr] lg:px-[40px] lg:pt-[170px]">
          <ul className="navigationMain__mainMenu flex flex-col gap-[10px]">
            <li className="font-S mb-[8px] font-semibold text-white">Our Focus</li>
          ...
          <div className="flex flex-col gap-[40px]">
            ...
            <div className="navigationMain__contactTwo flex flex-col gap-[6px]">
              <OfficeBlock office={OFFICES[1]} />
            </div>
            <div className="navigationMain__social flex flex-row flex-wrap gap-[24px]">
