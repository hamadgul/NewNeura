...
/**
 * `BlockIntroDouble` — the two-column intro that opens /about/, /culture/,
 * every /markets/ page and each project detail page (12 instances in all).
 *
 * Anatomy (identical in all three captured instances):
 *   section.blockIntroDouble        .lpas-grid, margin 60px 0 (50px < 1280px)
 ...
 * (/about/, statement only), 509px (/markets/housing/) and 487px (the project
 ...
 * (`.lpas-grid` ≥1280px; subgrid line 1 == main-start):
 ...
 *     on /markets/housing/ only resolves as 3 statement lines + 50px + 14 body
...
export function BlockIntroDouble({
  ...
  useEffect(() => {
    ...
    // Observing a `.lpas-reveal` child would be fine too — none of them are
  ...
  const revealClass = cn("lpas-reveal", isRevealed && "is-revealed");
  ...
  return (
    <section
      ref={sectionRef}
      className={cn(
        "blockIntroDouble lpas-grid my-[50px] overflow-clip text-[#111111] xl:my-[60px]",
        className,
      )}
    >
      <div
            ...
            const content = (
              <>
                ...
                {isActive ? (
                  ...
                  <span
                    aria-hidden
                    className="blockIntroDouble__headerLine absolute left-0 top-[calc(100%+10px)] h-px w-full bg-[var(--lpas-ink)]"
                  />
                ) : null}
              </>
            );
