 ...
 *
 * Measured grid placement (`.lpas-grid` line numbers):
 *
...
export interface BlockMediaDoubleQuoteVideo {
  type: "video";
  /** Local asset path, e.g. `/sites/lpas-com-76f4f1fd/<page-key>/videos/<file>.mp4`. */
  src: string;
...
export function BlockMediaDoubleQuote({
  ...
  useEffect(() => {
    // Observe the block root, never the `.lpas-image-reveal` container: that
  ...
  const revealClass = cn("lpas-image-reveal", isRevealed && "is-revealed");

  return (
    <div
      ref={rootRef}
      className={cn(
        "blockMediaDoubleQuote lpas-grid my-[100px] text-[#111111] xl:my-[120px]",
        className,
      )}
    >
      <figure
