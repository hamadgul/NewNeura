 ...
 *
 * Shared by all eight market pages (civic, commercial, interiors,
 * higher-education and the four housing sub-markets), so every string and
 ...
 * pin here is pure CSS: a sticky section plus a 1305px spacer sibling. No
...
/** The block only pins at the width where `.lpas-grid` becomes 20 columns. */
...
export interface ProcessPhase {
   ...
   * `/markets/housing/market-rate-housing/` the artwork luminance runs
...
export interface BlockProcessCardSliderProps {
  ...
  tagline: string;
  /** `__headerText` — "The LPAS affordable housing design process". */
  intro: string;
...
export function BlockProcessCardSlider({
  ...
  return (
    <div
      ref={pinRef}
      className="relative"
      style={{ "--lpas-pin": `${pinDistance}px` } as CSSProperties}
    >
      <section
        ref={sectionRef}
        data-control="BlockProcessCardSlider"
        className={cn(
          "blockProcessCardSlider lpas-grid relative overflow-x-clip",
      ...
      */}
      <div aria-hidden="true" className="motion-safe:xl:h-[var(--lpas-pin)]" />
    </div>
  );
}
 ...
 * are independent props because the source diverges on market-rate-housing.
 * ------------------------------------------------------------------ */
