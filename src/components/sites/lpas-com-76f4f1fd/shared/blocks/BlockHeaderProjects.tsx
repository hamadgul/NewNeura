 ...
 * `docs/research/lpas-com-76f4f1fd/shared-blocks/BlockHeaderProjects.styles.md`)
 ...
 * the main columns in half; `.lpas-grid` in globals.css carries the tracks but
 ...
 * `.lpas-image-reveal` IntersectionObserver trap does not apply here.
 */
import Image from "next/image";
import Link from "next/link";
import { cn } from "@/lib/utils";
import { ButtonArrow } from "@/components/sites/lpas-com-76f4f1fd/shared/buttons";
import { ChevronIcon } from "@/components/sites/lpas-com-76f4f1fd/shared/icons";
...
export interface BlockHeaderProjectsProps {
  ...
  title: string;
  /** One-line project summary shown opposite the back link. */
  lead: string;
  /** e.g. "Livermore, CA". */
  location: string;
  /** Market label, e.g. "Higher Education". */
  market: string;
  image: BlockHeaderProjectsImage;
  breadcrumbLabel?: string;
  breadcrumbHref?: string;
  backLabel?: string;
  /** Defaults to `breadcrumbHref` — both point at the portfolio index. */
  backHref?: string;
...
export function BlockHeaderProjects({
  ...
  return (
    <header
      className={cn(
        "blockHeaderProjects lpas-grid relative mb-[100px] h-[1170px] max-h-[130vh] grid-rows-[minmax(0,1fr)_minmax(0,0.3fr)] overflow-hidden",
        className,
      )}
    >
      {/*
      ...
      */}
      ...
      {/*
        Rows: 1fr (breadcrumb + back link + lead) / auto (location + market) /
      ...
      */}
      <div className="blockHeaderProjects__contentWrapper col-span-full row-start-1 z-[3] grid grid-cols-subgrid grid-rows-[auto_auto_1fr] text-white md:grid-rows-[minmax(0,1fr)_auto_auto]">
        ...
        {/*
          ...
          market line. (The extracted `align-items: end` on this row is not what
        ...
        */}
        ...
        <div className="blockHeaderProjects__market font-S col-start-2 col-end-[6] row-start-2 justify-self-end md:col-end-[14] xl:col-start-[18] xl:col-end-[22]">
          {market}
        </div>
