/**
 * `BlockHeaderGeneral` — the page header shared by /about/ and /culture/.
 *
 ...
 *
 * It is a `.lpas-grid` (20 main columns of 57.5px + 10px gaps at
 * 1440). The source places its two children with named grid areas
 * (`left` / `right`); since `.lpas-grid` publishes line names rather than
 ...
 * culture's 415 / 0 / 140 — different intro heights, identical 555px sum
...
export interface BlockHeaderGeneralImage {
  /** Local path under `/public`, e.g. `/sites/lpas-com-76f4f1fd/<page>/images/<file>`. */
  src: string;
...
export interface BlockHeaderGeneralProps {
  /** `<h1>` text — "About" / "Culture". */
  title: string;
  /**
   * Body copy in the second row. Optional: /culture/ renders the wrapper
  ...
  return (
    <header
      className={cn(
        ...
        "blockHeaderGeneral lpas-grid mb-[100px] h-[120vh] grid-rows-[7fr_5fr]",
