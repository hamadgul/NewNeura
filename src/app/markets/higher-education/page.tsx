import {
  ...
  PROCESS,
  PROJECTS,
  WYSIWYG_CLIENT_VOICES,
  WYSIWYG_WHY_LPAS,
} from "@/components/sites/lpas-com-76f4f1fd/markets-higher-education-95395586/content";
...
/**
 * The source serves "LPAS — Higher Education" (em dash, no suffix), which does
 * not fit the root layout's "%s - LPAS Architecture" template, so this route
 * opts out with an absolute title exactly as `/markets/housing/` does.
 *
...
/**
 * `/markets/higher-education/` — the parent Higher Education market page.
 *
 * Six blocks, in the order measured from the live DOM
 * (`markets-higher-education-95395586/RECON.json`, whose section list is the
 * authority here — BLOCKS.json misses the slider because GSAP wraps it in a
 * `pin-spacer` that hides the `data-control` attribute one level down):
 ...
 */
export default function MarketsHigherEducationPage() {
  return (
    <>
      <MainNavigation />
      ...
      <main className="mainContent relative w-full overflow-x-clip bg-white">
        ...
        <BlockWysiwyg {...WYSIWYG_CLIENT_VOICES} />
